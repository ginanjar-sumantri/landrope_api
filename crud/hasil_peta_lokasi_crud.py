...
from schemas.hasil_peta_lokasi_sch import (HasilPetaLokasiCreateSch, HasilPetaLokasiUpdateSch, 
                                        HasilPetaLokasiReadySpkSch, HasilPetaLokasiReadySpkExtSch, JenisBayarOnReady, HasilPetaLokasiUpdateCloud)
...
class CRUDHasilPetaLokasi(CRUDBase[HasilPetaLokasi, HasilPetaLokasiCreateSch, HasilPetaLokasiUpdateSch]):
    ...
    async def get_ready_spk(self, keyword: str | None = None) -> list[HasilPetaLokasiReadySpkSch]:
        ...
        query = f"""
                with subquery as (
                        select
                            ...
                            b.skpt_id,
                            hd.id as kjb_hd_id,
                            hpl.id as hasil_peta_lokasi_id,
                            hpl.request_peta_lokasi_id,
                            dt.id as kjb_dt_id
                        ...
                        Where 
                            NOT EXISTS (select 1 from checklist_kelengkapan_dokumen_dt c_dt
                                        ...
                                        inner join bundle_dt b_dt ON b_dt.id = c_dt.bundle_dt_id and b_dt.file_path is NULL
        ...
        objs = [HasilPetaLokasiReadySpkSch(id=row.id,
                                        id_bidang=row.id_bidang,
                                        alashak=row.alashak,
                                        kjb_termin_id=row.kjb_termin_id,
                                        ...
                                        kjb_harga_id=row.kjb_harga_id,
                                        pemilik_id=row.pemilik_id,
                                        skpt_id=row.skpt_id,
                                        kjb_hd_id=row.kjb_hd_id,
                                        hasil_petlok_id=row.hasil_peta_lokasi_id,
                                        req_petlok_id=row.request_peta_lokasi_id,
                                        kjb_dt_id=row.kjb_dt_id, 
