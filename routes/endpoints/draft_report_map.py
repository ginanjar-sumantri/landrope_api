...
from schemas.draft_report_map_sch import (DraftReportMapSch, DraftReportMapCreateSch, DraftReportMapUpdateSch, DraftReportMapHdCreateSch)
...
async def create(
    ...
    obj_currents = await crud.draft_report_map.get_multi_by_report_id(report_id=sch.report_id)

    if obj_currents:
        query = DraftReportMap.__table__.delete().where(DraftReportMap.report_id == sch.report_id)
        await crud.draft_report_map.delete(query=query, db_session=db_session, with_commit=False)

    drafts = []
    for dt in sch.details:
      draft_sch = DraftReportMapCreateSch(report_id=sch.report_id, project_id=dt.project_id, desa_id=dt.desa_id, ptsk_id=dt.ptsk_id, bidang_id=dt.bidang_id)
